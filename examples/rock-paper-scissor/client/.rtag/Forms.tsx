...
function PlayerInfoInput({ value, update }: BaseProps<PlayerInfo>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      ...
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">gesture</label>
        <OptionalInput<Gesture> value={ value.gesture } update={(v) => update({ ...value, gesture: v })} initialVal={ 0 }>
          {(value, update) =>
            <EnumInput value={value} update={update} enumType={ Gesture } />
          }
        </OptionalInput>
      </div>
    </div>
...
function PlayerStateInput({ value, update }: BaseProps<PlayerState>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">round</label>
        <IntInput value={ value.round } update={(v) => update({ ...value, round: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">player1</label>
        <PlayerInfoInput value={ value.player1 } update={(v) => update({ ...value, player1: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">player2</label>
        <OptionalInput<PlayerInfo> value={ value.player2 } update={(v) => update({ ...value, player2: v })} initialVal={ PlayerInfo.default() }>
          {(value, update) =>
                ...
                <div className="flex flex-col h-full py-6 overflow-y-scroll bg-white shadow-xl">
                  ...
                  <div className="relative flex-1 px-4 mt-6 sm:px-6">
                    <div className="forms">
                      ...
                      <MethodForm<IChooseGestureRequest>
                        method="chooseGesture"
                        submit={connection.chooseGesture.bind(connection)}
                        initialize={ IChooseGestureRequest.default }
                      >
                        {(value, update) => (
                          <div className="grid grid-cols-6 gap-2 pl-2 ml-2 border-l border-gray-300">
                            <div className="col-span-6 sm:col-span-4">
                              <label className="block mt-2 mb-1 text-sm font-medium text-gray-700">gesture</label>
                              <EnumInput value={ value.gesture } update={(v) => update({ ...value, gesture: v })} enumType={ Gesture } />
                            </div>
                          </div>
                        )}
                      </MethodForm>
                      <MethodForm<INextRoundRequest>
                        method="nextRound"
                        submit={connection.nextRound.bind(connection)}
                        initialize={ INextRoundRequest.default }
                      >
                        {(value, update) => (
                          <div className="grid grid-cols-6 gap-2 pl-2 ml-2 border-l border-gray-300">
                          </div>
