...
function MessageDisplay({ value, isCollapsible }: { value: Message, isCollapsible: boolean  }) {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  let icon;
  if (isCollapsed) {
    icon = <PlusSmIcon className="w-3 h-3 fill-current" aria-hidden="true" />;
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="text">
            <StringDisplay value={ value.text } />
          </KVDisplay>
          <KVDisplay label="sentAt">
            <IntDisplay value={ value.sentAt } />
          </KVDisplay>
          <KVDisplay label="sentBy">
            <UserIdDisplay value={ value.sentBy } />
          </KVDisplay>
          <KVDisplay label="sentTo">
            <OptionalDisplay<UserId> value={ value.sentTo }>
              {(value) => 
...
function RoomStateDisplay({ value, isCollapsible }: { value: RoomState, isCollapsible: boolean  }) {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  let icon;
  if (isCollapsed) {
    icon = <PlusSmIcon className="w-3 h-3 fill-current" aria-hidden="true" />;
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="name">
            <StringDisplay value={ value.name } />
          </KVDisplay>
          <KVDisplay label="createdBy">
            <UserIdDisplay value={ value.createdBy } />
          </KVDisplay>
          <KVDisplay label="messages">
            <ArrayDisplay<Message> value={ value.messages }>
              {(value) => 
                <MessageDisplay value={value} isCollapsible={ true } />
              }
            </ArrayDisplay>
          </KVDisplay>
        </div>
...
export function State() {
  ...
  return (
    <div className="w-full font-mono text-gray-700 state-display">
      <RoomStateDisplay value={value} isCollapsible={ false } />
    </div>
  );
}
