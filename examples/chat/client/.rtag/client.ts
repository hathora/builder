...
export class RtagClient {
  ...
  public connectNew(
    ...
    return new Promise((resolve) => {
      ...
      socket.onopen = () => {
        socket.send(token);
        socket.send(ICreateRoomRequest.encode(request).toBuffer());
      };
...
export class RtagConnection {
  ...
  public sendPublicMessage(request: ISendPublicMessageRequest): Promise<Response> {
    return this.callMethod(Method.SEND_PUBLIC_MESSAGE, ISendPublicMessageRequest.encode(request).toBuffer());
  }

  public sendPrivateMessage(request: ISendPrivateMessageRequest): Promise<Response> {
    return this.callMethod(Method.SEND_PRIVATE_MESSAGE, ISendPrivateMessageRequest.encode(request).toBuffer());
  }
