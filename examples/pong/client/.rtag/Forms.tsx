...
function PointInput({ value, update }: BaseProps<Point>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">x</label>
        <FloatInput value={ value.x } update={(v) => update({ ...value, x: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">y</label>
        <FloatInput value={ value.y } update={(v) => update({ ...value, y: v })} />
      </div>
    </div>
  );
}

function PlayerInput({ value, update }: BaseProps<Player>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">paddle</label>
        <FloatInput value={ value.paddle } update={(v) => update({ ...value, paddle: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">score</label>
        <IntInput value={ value.score } update={(v) => update({ ...value, score: v })} />
      </div>
    </div>
  );
}
...
function PlayerStateInput({ value, update }: BaseProps<PlayerState>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">playerA</label>
        <PlayerInput value={ value.playerA } update={(v) => update({ ...value, playerA: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">playerB</label>
        <PlayerInput value={ value.playerB } update={(v) => update({ ...value, playerB: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">ball</label>
        <PointInput value={ value.ball } update={(v) => update({ ...value, ball: v })} />
      </div>
    </div>
  );
}
              ...
              <div className="relative w-screen max-w-lg">
                <div className="flex flex-col h-full py-6 overflow-y-scroll bg-white shadow-xl">
                  ...
                  <div className="relative flex-1 px-4 mt-6 sm:px-6">
                    <div className="forms">
                      <MethodForm<ISetDirectionRequest>
                        method="setDirection"
                        submit={connection.setDirection.bind(connection)}
                        initialize={ ISetDirectionRequest.default }
                      >
                        {(value, update) => (
                          <div className="grid grid-cols-6 gap-2 pl-2 ml-2 border-l border-gray-300">
                            <div className="col-span-6 sm:col-span-4">
                              <label className="block mt-2 mb-1 text-sm font-medium text-gray-700">direction</label>
                              <EnumInput value={ value.direction } update={(v) => update({ ...value, direction: v })} enumType={ Direction } />
                            </div>
                          </div>
                        )}
                      </MethodForm>
