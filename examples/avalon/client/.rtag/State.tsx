...
function RoleInfoDisplay({ value, isCollapsible }: { value: RoleInfo, isCollapsible: boolean  }) {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  let icon;
  if (isCollapsed) {
    icon = <PlusSmIcon className="w-3 h-3 fill-current" aria-hidden="true" />;
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="role">
            <EnumDisplay value={ value.role } enum={ Role } />
          </KVDisplay>
          <KVDisplay label="isEvil">
            <BooleanDisplay value={ value.isEvil } />
          </KVDisplay>
          <KVDisplay label="knownRoles">
            <ArrayDisplay<Role> value={ value.knownRoles }>
              {(value) => 
                <EnumDisplay value={value} enum={ Role } />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="quantity">
            <IntDisplay value={ value.quantity } />
          </KVDisplay>
        </div>
      ) : (
        <div className="p-1 m-1 bg-white border rounded object-display">
...
function PlayerAndVoteDisplay({ value, isCollapsible }: { value: PlayerAndVote, isCollapsible: boolean  }) {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  let icon;
  if (isCollapsed) {
    icon = <PlusSmIcon className="w-3 h-3 fill-current" aria-hidden="true" />;
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="player">
            <UserIdDisplay value={ value.player } />
          </KVDisplay>
          <KVDisplay label="vote">
            <OptionalDisplay<Vote> value={ value.vote }>
              {(value) => 
                <EnumDisplay value={value} enum={ Vote } />
              }
            </OptionalDisplay>
          </KVDisplay>
        </div>
...
function QuestAttemptDisplay({ value, isCollapsible }: { value: QuestAttempt, isCollapsible: boolean  }) {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  let icon;
  if (isCollapsed) {
    icon = <PlusSmIcon className="w-3 h-3 fill-current" aria-hidden="true" />;
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="id">
            <IntDisplay value={ value.id } />
          </KVDisplay>
          <KVDisplay label="status">
            <EnumDisplay value={ value.status } enum={ QuestStatus } />
          </KVDisplay>
          <KVDisplay label="roundNumber">
            <IntDisplay value={ value.roundNumber } />
          </KVDisplay>
          <KVDisplay label="attemptNumber">
            <IntDisplay value={ value.attemptNumber } />
          </KVDisplay>
          <KVDisplay label="leader">
            <UserIdDisplay value={ value.leader } />
          </KVDisplay>
          <KVDisplay label="members">
            <ArrayDisplay<UserId> value={ value.members }>
              {(value) => 
                <UserIdDisplay value={value} />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="proposalVotes">
            <ArrayDisplay<PlayerAndVote> value={ value.proposalVotes }>
              {(value) => 
                <PlayerAndVoteDisplay value={value} isCollapsible={ true } />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="results">
            <ArrayDisplay<PlayerAndVote> value={ value.results }>
              {(value) => 
                <PlayerAndVoteDisplay value={value} isCollapsible={ true } />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="numFailures">
            <IntDisplay value={ value.numFailures } />
          </KVDisplay>
        </div>
      ) : (
        <div className="p-1 m-1 bg-white border rounded object-display">
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="status">
            <EnumDisplay value={ value.status } enum={ GameStatus } />
          </KVDisplay>
          <KVDisplay label="rolesInfo">
            <ArrayDisplay<RoleInfo> value={ value.rolesInfo }>
              {(value) => 
                <RoleInfoDisplay value={value} isCollapsible={ true } />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="creator">
            <UserIdDisplay value={ value.creator } />
          </KVDisplay>
          <KVDisplay label="players">
            <ArrayDisplay<UserId> value={ value.players }>
              {(value) => 
                <UserIdDisplay value={value} />
              }
            </ArrayDisplay>
          ...
          <KVDisplay label="role">
            <OptionalDisplay<Role> value={ value.role }>
              {(value) => 
                <EnumDisplay value={value} enum={ Role } />
              }
            </OptionalDisplay>
          </KVDisplay>
          <KVDisplay label="knownPlayers">
            <ArrayDisplay<UserId> value={ value.knownPlayers }>
              {(value) => 
                <UserIdDisplay value={value} />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="playersPerQuest">
            <ArrayDisplay<number> value={ value.playersPerQuest }>
              {(value) => 
                <IntDisplay value={value} />
              }
            </ArrayDisplay>
          </KVDisplay>
          <KVDisplay label="quests">
            <ArrayDisplay<QuestAttempt> value={ value.quests }>
              {(value) => 
                <QuestAttemptDisplay value={value} isCollapsible={ true } />
              }
            </ArrayDisplay>
          </KVDisplay>
        </div>
