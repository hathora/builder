...
function PieceDisplay({ value, isCollapsible }: { value: Piece, isCollapsible: boolean  }) {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);
  let icon;
  if (isCollapsed) {
    icon = <PlusSmIcon className="w-3 h-3 fill-current" aria-hidden="true" />;
      ...
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="color">
            <EnumDisplay value={ value.color } enum={ Color } />
          </KVDisplay>
          <KVDisplay label="type">
            <EnumDisplay value={ value.type } enum={ PieceType } />
          </KVDisplay>
          <KVDisplay label="square">
    ...
    <>
      {!isCollapsed ? (
        <div className="p-1 m-1 bg-white border rounded object-display">
          ...
          <KVDisplay label="board">
            <PluginDisplay value={ value.board } component="board-plugin" isCollapsible={ true } />
          </KVDisplay>
          ...
          <KVDisplay label="opponent">
            <OptionalDisplay<UserId> value={ value.opponent }>
              {(value) => 
