...
export class RtagConnection {
  ...
  public fold(request: IFoldRequest): Promise<Response> {
    return this.callMethod(Method.FOLD, IFoldRequest.encode(request).toBuffer());
  }

  public call(request: ICallRequest): Promise<Response> {
    return this.callMethod(Method.CALL, ICallRequest.encode(request).toBuffer());
  }

  public raise(request: IRaiseRequest): Promise<Response> {
    return this.callMethod(Method.RAISE, IRaiseRequest.encode(request).toBuffer());
  }
